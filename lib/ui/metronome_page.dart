...
class _MetronomePageState extends State<MetronomePage>
  ...
  double _bpmToAngle(int bpm) {
    ...
    final double p =
        ((bpm - minBpm) / (maxBpm - minBpm)).clamp(0.0, 1.0);
  ...
  static const double _arcStart = -210 * math.pi / 180;
  static const double _arcSweep =  240 * math.pi / 180;

  void _onCircularDragUpdate(
      DragUpdateDetails details, Offset widgetCentre) {
    ...
    if (relative > _arcSweep) {
      final double distToStart = relative - _arcSweep;
      final double distToEnd   = 2 * math.pi - relative;
      relative = distToStart < distToEnd ? _arcSweep : 0.0;
    }
    final double fraction =
        (relative / _arcSweep).clamp(0.0, 1.0);
    final int newBpm =
        (30 + fraction * (240 - 30)).round().clamp(30, 240);
    if (newBpm != _bpm) {
      setState(() {
        _bpm = newBpm;
        _bpmTextController.text = newBpm.toString();
        _updateNeedleAnimation(_bpm, instant: true);
      });
    }
  }
  ...
  Widget build(BuildContext context) {
    // Use Column directly inside a non-scrollable SafeArea.
    // The dial + controls are fixed-height so no scrolling is needed
    // and the GestureDetector over the dial is never intercepted.
    return Scaffold(
                ...
                child: LayoutBuilder(
                  builder: (context, constraints) {
                    ...
                    return GestureDetector(
                      onPanUpdate: (d) =>
                          _onCircularDragUpdate(d, centre),
                  ...
                  children: [
                    ...
                    Row(
                      ...
                      children: [1, 2, 3, 4].map((n) {
                        final bool sel = _numerator == n;
                        return GestureDetector(
                          onTap: () => setState(() => _numerator = n),
    ...
    for (int i = 0; i < numerator; i++) {
      ...
      if (isActive) {
        canvas.drawCircle(
          pos, dotR,
          Paint()
            ..color = (isAccent ? Colors.redAccent : Colors.greenAccent)
                .withOpacity(0.5)
            ..style       = PaintingStyle.stroke
            ..strokeWidth = 4
            ..maskFilter  =
